...
from geotransformer.utils.common import load_pickle, load_metadata
...
class CustomPairDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.metadata = load_metadata(self.dataset_root, CUSTOM_DATA_SPLIT[subset])
        if self.subset == 'infer' or self.subset == 'val' or self.subset == 'test':
            self.metadata = random.sample(self.metadata, 10000)
        if self.subset == 'train':
            self.metadata = random.sample(self.metadata, 50000)
    ...
    def __getitem__(self, index):
        ...
        data_dict['ref_frame'] = metadata['frame1']
        data_dict['src_frame'] = metadata['frame0']
